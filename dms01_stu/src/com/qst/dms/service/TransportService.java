		...
		public void saveMatchTransportToDB(ArrayList<MatchedTransport> matchTrans){
			...
			try {
				...
				for(MatchedTransport matchedTransport : matchTrans) {
					...
					int Psend=db.executeSQLAndReturnPrimarKey(sql, param);
					send.setId(Psend);
					//保存匹配记录中的运输状态
					param = new Object[] { 
					...
					int ptrans=db.executeSQLAndReturnPrimarKey(sql, param);
					trans.setId(ptrans);
					//保存匹配记录中的接收状态
					param = new Object[] { 
					...
					int preceive=db.executeSQLAndReturnPrimarKey(sql, param);
					receive.setId(preceive);
					//保存匹配日志的ID
