			...
			while (true) {
				...
				case 3:
					...
					if(type==1) {
						...
						//保存匹配的物流信息到文件中
						tranService.saveAndAppandMatchedTransport(matchedTrans);
				...
				case 4: {
					...
					ArrayList<MatchedLogRec>matchedLogsDB=logService.readMatchedLogFromDB();
					ArrayList<MatchedTransport> matchedTransDB=tranService.readMatchedTransportFromDB();
					if (matchedLogsDB== null || matchedLogsDB.size() == 0) {
						System.out.println("匹配的日志记录是0条！");
					} else {
						//输出匹配的日志信息
						System.out.println("从数据库中匹配的日志信息有：");
						logService.showMatchLog(matchedLogsDB);
						matchedLogsDB.clear();
						}
					if (matchedTransDB == null || matchedTransDB.size() == 0) {
						System.out.println("匹配的物流记录是0条！");
					} else {
						// 输出匹配的物流信息
						System.out.println("从数据库中匹配的物流数据有：");
						tranService.showMatchTransport(matchedTransDB);
						matchedTransDB.clear();
					}
				}
					break;
				case 5:
