...
import com.qst.dms.entity.MatchedBetter;
...
public class MainFrame extends JFrame {
	...
	private void initToolBar() {
		...
		//ImageIcon tranIcon = new ImageIcon("images\\transData.png");
		btnMatchTrans = new JButton("匹配物流数据", null);
		// 注册监听
	...
	private void initTransGatherGUI() {
		...
		cmbTranStatus = new JComboBox(tranStatus);
	...
	private class GatherLogListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			// 获取日志ID
			int id = 0;//Integer.parseInt(txtLogId.getText().trim());
			// 创建当前时间
			Date time = new Date();
			// 获取地址栏地址
	...
	private class GatherTransListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			// 获取物流ID
			int id = 0;//Integer.parseInt(txtTransId.getText().trim());
			// 创建当前时间
			Date time = new Date();
			// 获取地址栏地址
		...
		public void actionPerformed(ActionEvent e) {
			...
			// 保存匹配的日志信息
						...
						if(matchedTrans != null && matchedTrans.size() > 0) {
							// 匹配的物流信息保存到文件
							transportService.saveAndAppandMatchedTransport(matchedTrans);
							// 匹配的物流信息保存到数据库中
							...
							// 保存成功，弹出提示框：匹配的物流数据已以保存到文件和数据库中！
		...
		private class SendDataListener implements ActionListener{
			...
			public void actionPerformed (ActionEvent e) {
				try {
					//判断匹配的日志信息列表是否不为空
					if (matchedLogs!=null && matchedLogs.size()>0) {
						//创建Socket发送日志信息，日志信息发送到服务器的6666湍口
					...
					if (matchedTrans!=null && matchedTrans.size()>0) {
						//创建Socket发送物流信息，物流信息发送到服务器的6668湍口
	...
	//线程类,每隔1分钟刷新━次显示数据表格中的数据.与数据库保持同步
	...
	private void flushMatchedLogTable() {
		...
		MatchedBetter logModel=new MatchedBetter(1);
	...
	private void flushMatchedTransTable() {
		//显示物流数据表格
		// 创建tableModel，通过标志为区分日志和物流：1，日志；0，物流
		MatchedBetter transModel=new MatchedBetter(0);
		...
		DefaultTableCellRenderer r = new DefaultTableCellRenderer();
		r.setHorizontalAlignment(JLabel.CENTER);
		transTable.setDefaultRenderer(Object.class, r);
		// 通过JTable对象创建JScrollPane，显示数据
